...
namespace ToolGood.Algorithm
{
    ...
    {'蓝色':'XXL','canBookCount':985,'saleCount':14,'specId':'358b6c3b52bf711ac8ecfe7513a4f3ad'}
        ...
        {
            AlgorithmEngine engine = new AlgorithmEngine();
            var num = engine.TryEvaluate("lookup({2:1,2:2,4:3,2:4,5:5},2)", 0);
            Assert.AreEqual(num, 1);

            num = engine.TryEvaluate("lookup({2:1,2:2,4:3,2:4,5:5},4)", 0);
            Assert.AreEqual(num, 3);

            num = engine.TryEvaluate("lookup({2:1,2:2,4:3,2:4,5:5},2.1)", 0);
            Assert.AreEqual(num, 0);

            num = engine.TryEvaluate("lookup({2:1,2:2,4:3,2:4,5:5},2.1,true)", 0);
            Assert.AreEqual(num, 2);

            num = engine.TryEvaluate("lookup({2:1,2:2,4:3,2:4,5:5},4.1,true)", 0);
            Assert.AreEqual(num, 4);


            num = engine.TryEvaluate("lookup({2:1,2:2,4:3,2:4,5:5},6.1,true)", 0);
