...
namespace ToolGood.Algorithm
{
    ...
    class AlgorithmEngineTest
    {
        ...
        public void Test_0()
        {
            ...
            var c = engine.TryEvaluate("if(2+3*2+10/2*4,1", 0);
            Assert.AreEqual(0, c);
        }
        ...
        public void Test()
        {
            ...
            c = engine.TryEvaluate("(2)+0", 0);
            Assert.AreEqual(2, c);
            c = engine.TryEvaluate("0+2", 0);
            Assert.AreEqual(2, c);
            c = engine.TryEvaluate("1*2", 0);
            Assert.AreEqual(2, c);
            c = engine.TryEvaluate("2*1", 0);
            Assert.AreEqual(2, c);
            c = engine.TryEvaluate("2/1", 0);
            Assert.AreEqual(2, c);
            c = engine.TryEvaluate("(2)-0", 0);
            ...
            t1 = engine.TryEvaluate("1m>50cm？1：2", 0);
            Assert.AreEqual(t1, 1);

            t1 = engine.TryEvaluate("1m+2>50cm？1：2", 0);
            Assert.AreEqual(t1, 1);

            t1 = engine.TryEvaluate("1m+2cm>50cm？1：2", 0);
            Assert.AreEqual(t1, 1);

            t1 = engine.TryEvaluate("1m*2cm>50cm？1：2", 0);
            Assert.AreEqual(t1, 2);

            d = engine.TryEvaluate("1m+2cm", 0.0);
            Assert.AreEqual(d, 1.02);


            d = engine.TryEvaluate("'true'*3", 0.0);
            Assert.AreEqual(d, 3);
        ...
        [Test]
        public void base_test6()
        {
            AlgorithmEngine engine = new AlgorithmEngine();
            engine.UseExcelIndex = false;

            String c = engine.TryEvaluate("{'1',10:'2','q':'3',}[0]", "");
            Assert.AreEqual("1", c);

            c = engine.TryEvaluate("{'1',10:'2','q':'3'}[10]", "");
            Assert.AreEqual("2", c);

            c = engine.TryEvaluate("{'1',10:'2','q':'3'}[11]", "");
            Assert.AreEqual("3", c);

            c = engine.TryEvaluate("{'1',10:'2','q':'3'}['q']", "");
            Assert.AreEqual("3", c);

            var d = engine.TryEvaluate("{'1',10:'2','q':'3'}.has('3')", true);
            Assert.AreEqual(true, d);

            d = engine.TryEvaluate("'3'.in({'1',10:'2','q':'3'})", true);
            Assert.AreEqual(true, d);
