    ...
    public sealed class VerifyCodeUtil
    {
        ...
        public byte[] CreateImageBytes(string code)
        {
            ...
            image.Mutate(ctx => {
                ...
                // 画干扰线
                for (int i = 0; i < 6; i++) {
                    var pen = new SolidPen(NoiseColors[r.Next(NoiseColors.Count)], 2);
                ...
                // 画噪点
                for (int i = 0; i < 150; i++) {
                    var pen = new SolidPen(ShowColors[r.Next(ShowColors.Count)], 1);
                    var p1 = new PointF(r.Next(Width), r.Next(Height));
                    var p2 = new PointF(p1.X + 1f, p1.Y + 1f);

                    ctx.DrawLine(pen, p1, p2);
                }
                var pen2 = new SolidPen(Color.Gainsboro, 1);
