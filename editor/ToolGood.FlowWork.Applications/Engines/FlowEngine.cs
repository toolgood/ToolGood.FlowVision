...
namespace ToolGood.FlowWork.Applications.Engines
{
    public class FlowEngine : IDisposable
    {
        ...
        #endregion 初始化 Project MachineSetting HolidaySetting
        ...
        internal void BindingPreviousJson(string previous)
        {
            ...
            previous = JObject.Parse(previous).ToString();
            _initDict["上个流程"] = Operand.CreateJson(previous);
        }

        #endregion BindingPreviousJson
        ...
        #endregion EvaluateInputNum 计算入量

        #region GetProcedureDetail 获取 机械工艺 及 公式

        public ProcedureDetail GetProcedureDetail(List<FormulaMatch> formulaMatches)
        {
        ...
        internal List<IFormulaItem> GetFormulaItems(string formula)
        {
            if (string.IsNullOrEmpty(formula)) { return null; }

            List<IFormulaItem> result = new List<IFormulaItem>();
            var diyNameInfo = AlgorithmEngineHelper.GetDiyNames(formula);
            ...
            for (int i = 0; i < diyNameInfo.Parameters.Count; i++) {
                ...
                formulaItem.Name = parameter.Name;
                var operand = GetTempParameter(parameter.Name);
                if (operand.IsError) {
                    formulaItem.Value = $"error(\"{operand.ErrorMsg}\")";
                } else if (operand.Type == OperandType.TEXT) {
                    formulaItem.Value = $"\"{operand.TextValue}\"";
                } else {
                    formulaItem.Value = operand.ToText().TextValue;
                }
        ...
        internal void GetFormulaItems2(List<IFormulaItem> formulaNames, Dictionary<string, FormulaDetailItem> temp, int layer)
        {
            for (int i = 0; i < formulaNames.Count; i++) {
                var formulaItem = formulaNames[i];
                if (formulaItem is TextItem) { continue; }
                var formula = formulaItem as FormulaItem;
                if (temp.TryGetValue(formula.Name, out FormulaDetailItem detailItem)) {
                    if (detailItem.Layer < layer) { detailItem.Layer = layer; }
                    continue;
                }
                if (formula.Name == "出量") { continue; }
                bool hasOutputNum = false; // 每一个节点都有出量，为了保证出量正确，计算前复制出量
                if (_outputNumDict.TryGetValue(formula.Name, out TreeNode treeNode)) {
                    hasOutputNum = true;
                    SetOutputNum(treeNode.OutputNum.Value);
                }

                FormulaDetailItem formulaDetailItem = new FormulaDetailItem();
                formulaDetailItem.Name = formula.Name;
                formulaDetailItem.Value = formula.Value;
                formulaDetailItem.Layer = layer;

                string fma = null;
                if (_initDict.ContainsKey(formula.Name)) {
                    formulaDetailItem.Type = "initValue";
                    fma = AppWork.InitValueList.First(q => q.Name == formula.Name).GetMatchFormula(this);
                } else if (_inputDict.ContainsKey(formula.Name)) {
                    formulaDetailItem.Type = "input";
                } else if (_progDict.ContainsKey(formula.Name)) {
                    formulaDetailItem.Type = "flow";
                    formulaDetailItem.NodeId = _progDict[formula.Name].NodeWork.Id;
                    fma = _progDict[formula.Name].GetMatchFormula(this);
                ...
                } else if (_inputNumDict.ContainsKey(formula.Name)) {
                    if (_inputNumDict[formula.Name].CurrWork is IInputFormulaNodeWork procedure) {
                        formulaDetailItem.Type = "flow";
                        formulaDetailItem.NodeId = _inputNumDict[formula.Name].CurrWork.Id;
                        fma = procedure.GetMatchFormula(this);
                    }
                }
                if (fma != null) {
                    formulaDetailItem.Formulas = GetFormulaItems(fma);
                    GetFormulaItems2(formulaDetailItem.Formulas, temp, layer + 1);
                }
                temp[formula.Name] = formulaDetailItem;

                if (hasOutputNum) { ClearOutputNum(); } // 每一个节点都有出量，为了保证出量正确，计算后清除出量
            }
        }
        ...
        {
            return ToolGood.Algorithm.AlgorithmEngineHelper.Evaluate(context, GetTempParameter, null,
                    Project.ExcelIndex == 1, true, (DistanceUnitType)Project.Distance, (AreaUnitType)Project.Area, (VolumeUnitType)Project.Volume, (MassUnitType)Project.Mass
                    );
        }
        ...
        #endregion CheckMachine 检测机器是否可以使用
