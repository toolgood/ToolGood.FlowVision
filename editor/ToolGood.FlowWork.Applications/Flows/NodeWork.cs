...
namespace ToolGood.FlowWork.Flows
{
	...
	public abstract class NodeWork
	{
		...
		internal virtual string GetErrorMessage(FlowEngineEx engine, string factoryCode) => null;
	...
	public sealed class ErrorFlowWork : NodeWork
	{
		...
		internal override string GetErrorMessage(FlowEngineEx engine, string factoryCode)
		{
			return ErrorMessage;
		}
	}
		...
		internal override string GetErrorMessage(FlowEngineEx engine, string factoryCode)
		{
			...
			if (result == null) {
				if (InputFormula != null && InputFormula.Count > 0) {
					...
					for (int i = 0; i < InputFormula.Count; i++) {
						var inputFormula = InputFormula[i];
						var exp = inputFormula.Condition;
						var r = engine.FindErrorCondition(exp);
						if (r == null) {
							b = true;
						} else {
							errors.Add(r);
			...
			if (result == null) {
				if (CheckType == CheckType.Add) {
					result = FactoryProcedure.GetErrorMessage(engine, factoryCode);
				}
			}
			if (result == null) {
				if (FactoryProcedure.Items.ContainsKey(factoryCode) == false) {
					result = $"{factoryCode}厂区无此[{FactoryProcedure.Name}]工艺";
				}
			}
			return result;
		}
	...
	public sealed class ProcedureFlowMachineInfo
	{
		...
		internal string GetErrorMessage(FlowEngineEx engine)
		{
			HashSet<string> errors = new HashSet<string>();
			if (string.IsNullOrEmpty(Condition) == false) {
				var r = engine.FindErrorCondition(Condition);
				if (r != null) { errors.Add(r); }
			}
			if (CheckType == CheckType.Add) {
				var r = FactoryMachine.GetErrorMessage(engine);
				if (r != null) { errors.Add(r); }
			}
	...
	public sealed class CustomFlowWork : NodeWork
	{
		...
		internal override string GetErrorMessage(FlowEngineEx engine, string factoryCode)
		{
			return engine.FindErrorCondition(CheckFormula);
		}
	}
